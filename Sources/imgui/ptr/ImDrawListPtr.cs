...
public struct ImDrawListPtr
{
  ...
  public unsafe ImDrawListPtr(IntPtr nativePtr) => this.NativePtr = (ImDrawList*) nativePtr;
  ...
  {
    get => (IntPtr) this.NativePtr->_IdxWritePtr;
    set => this.NativePtr->_IdxWritePtr = (ushort*) value;
  }
  ...
  public unsafe void _ClearFreeMemory() => ImGuiNative.ImDrawList__ClearFreeMemory(this.NativePtr);
  ...
  public unsafe void AddConcavePolyFilled(ref Vector2 points, int num_points, uint col)
  {
    fixed (Vector2* points1 = &points)
      ImGuiNative.ImDrawList_AddConcavePolyFilled(this.NativePtr, points1, num_points, col);
  }
  ...
  public unsafe void AddEllipse(Vector2 center, Vector2 radius, uint col)
  {
    ...
    int num_segments = 0;
    float thickness = 1f;
    ImGuiNative.ImDrawList_AddEllipse(this.NativePtr, center, radius, col, rot, num_segments, thickness);
  }
  ...
  public unsafe void AddEllipseFilled(Vector2 center, Vector2 radius, uint col)
  {
    float rot = 0.0f;
    int num_segments = 0;
    ImGuiNative.ImDrawList_AddEllipseFilled(this.NativePtr, center, radius, col, rot, num_segments);
  }
  ...
  public unsafe void AddEllipseFilled(
    Vector2 center,
    Vector2 radius,
    uint col,
    float rot,
  ...
  public unsafe void AddText(Vector2 pos, uint col, ReadOnlySpan<char> text_begin)
  {
    int byteCount = Encoding.UTF8.GetByteCount(text_begin);
    byte* numPtr = byteCount <= 2048 /*0x0800*/ ? stackalloc byte[byteCount + 1] : Util.Allocate(byteCount + 1);
    int utf8 = Util.GetUtf8(text_begin, numPtr, byteCount);
    numPtr[utf8] = (byte) 0;
    ImGuiNative.ImDrawList_AddText_Vec2(this.NativePtr, pos, col, numPtr, numPtr + byteCount);
    if (byteCount <= 2048 /*0x0800*/)
      return;
    Util.Free(numPtr);
  }
    ...
    ReadOnlySpan<char> text_begin)
  {
    ...
    ImGuiNative.ImDrawList_AddText_FontPtr(this.NativePtr, nativePtr, font_size, pos, col, numPtr, numPtr + byteCount, wrap_width, cpu_fine_clip_rect);
    if (byteCount <= 2048 /*0x0800*/)
      return;
    Util.Free(numPtr);
  }
  ...
  public unsafe void AddText(
    ...
    ReadOnlySpan<char> text_begin,
    float wrap_width,
    ref Vector4 cpu_fine_clip_rect)
  {
    ImFont* nativePtr = font.NativePtr;
    int byteCount = Encoding.UTF8.GetByteCount(text_begin);
    byte* numPtr = byteCount <= 2048 /*0x0800*/ ? stackalloc byte[byteCount + 1] : Util.Allocate(byteCount + 1);
    int utf8 = Util.GetUtf8(text_begin, numPtr, byteCount);
    numPtr[utf8] = (byte) 0;
    fixed (Vector4* cpu_fine_clip_rect1 = &cpu_fine_clip_rect)
    {
      ImGuiNative.ImDrawList_AddText_FontPtr(this.NativePtr, nativePtr, font_size, pos, col, numPtr, numPtr + byteCount, wrap_width, cpu_fine_clip_rect1);
      if (byteCount > 2048 /*0x0800*/)
        Util.Free(numPtr);
    }
  }
  ...
  public unsafe void PathEllipticalArcTo(
    Vector2 center,
    Vector2 radius,
    float rot,
    float a_min,
  ...
  {
    int num_segments = 0;
    ImGuiNative.ImDrawList_PathEllipticalArcTo(this.NativePtr, center, radius, rot, a_min, a_max, num_segments);
  }
  ...
  {
    ImGuiNative.ImDrawList_PathFillConcave(this.NativePtr, col);
  }
  ...
  {
    byte intersect_with_current_clip_rect1 = (byte) intersect_with_current_clip_rect;
